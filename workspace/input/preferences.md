...
**MANDATORY**: Before running any Go commands that fetch dependencies, configure Git and Go for private repositories:

```bash
# Configure Go to treat these as private (not proxy.golang.org)
...
**Why This Is Needed**:
- `junbon-deriv` repositories are private and require SSH authentication
- `regentmarkets` module paths may map to `junbon-deriv` repositories
- Without `GOPRIVATE`, Go will try to fetch from proxy.golang.org and fail
- Without SSH URL rewriting, Go modules may fail to authenticate

**Module Path Mapping**:
When a dependency declares a different module path than its actual location, use `replace` directives:
```go
// In go.mod - map declared path to actual repository
...
# Test that Go can fetch private modules
go list -m github.com/junbon-deriv/service-feed@latest
```
